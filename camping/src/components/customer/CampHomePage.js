...
function CampHomePage() {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/campsites');
      const filtered = response.data.filter(campsite =>
        ...
        {filteredCampsites.map(campsite => (
          <li key={campsite.camp_id} className="campsite-item">
            <div className="campsite-info">
              ...
              <p><strong>Address:</strong> {campsite.camp_address}</p>
              <p><strong>Contact:</strong> {campsite.contact}</p>
            </div>
            <button onClick={() => goToDetailPage(campsite)}>상세정보</button>
          </li>
        ))}
      </ul>
      <button onClick={goToCustPage} className="my-page-button">마이페이지</button>
    </div>
  );
}
