...
const CustPage = () => {
    ...
    const [showBookings, setShowBookings] = useState(true); // State to control which section to show
        ...
        try {
            ...
            setBookings((prevBookings) =>
                prevBookings.map((booking) =>
                    booking.book_id === bookingId ? { ...booking, book_status: "예약취소" } : booking
                )
            );
            alert("예약이 취소되었습니다.");
        } catch (error) {
    ...
    return (
        <div className="container">
            ...
            {/* Buttons to toggle between Bookings and Reviews */}
                            ...
                            bookings.map((booking) => (
                                <div key={booking.book_id} className="booking">
                                    ...
                                    {booking.book_status === "예약대기" ? (
                                        <button onClick={() => handleCancelClick(booking.book_id)}>
                                            예약 취소
                                        </button>
                                    ) : booking.book_status === "예약종료" ? (
                                        <button onClick={() => handleReviewClick(booking.book_id)}>
                                            리뷰 작성
                                        </button>
                                    ) : (
                                        <button disabled>
                                            {booking.book_status === "예약취소" ? "예약 취소됨" : "예약 확정"}
                                        </button>
                                    )}
                                </div>
                            ))
                        ...
                        {reviews.length > 0 ? (
                            reviews.map((review) => (
                                <div key={review.review_id} className="review">
                                    ...
                                    <p><strong>내용:</strong> {review.comments}</p>
